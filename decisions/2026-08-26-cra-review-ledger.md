...
CRA batch reviewer의 command 완료 여부는 sentinel 파일이 아니라 blocking `codex review`의 exit status로 판정한다. Prompt, ledger, 고정 log는 계속 `git rev-parse --git-path cra` 아래에서 재사용하고, log redirection은 `>|`를 사용해 shell `noclobber`가 활성화된 환경에서도 매 pass 의도적으로 truncate한다. 기본 동작으로 pass별 임시 디렉터리나 실행 산출물을 누적하지 않는다.
...
8. ledger, prompt, 고정 log는 `git rev-parse --git-path cra` 아래에 두어 task commit과 worktree를 오염시키지 않는다.
...
10. custom review prompt를 지원하지 않는 환경에서는 같은 Sol·Max·long-context override와 blocking command·고정 log truncation·exit-status discipline을 유지한 `codex review --commit` 전체 검토로 안전하게 폴백한다. 명시적 unit coverage가 없으면 incremental reuse도 하지 않는다.
