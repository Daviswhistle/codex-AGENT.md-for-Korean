...
구조는 `references/genre-playbooks.md`에서 지배적인 목적에 맞는 플레이북을 고른다. 장문 또는 반복 발행되는 근거 기반 조사 에세이에서 중복·연대기·주장별 근거·인과 강도를 별도 관리할 실익이 크면 `references/evidence-backed-research-essay.md`를 적용한다. 짧은 출처 기반 Q&A에는 적용하지 않는다.
...
사실 오류, 논리 오류, 불명확성, 리듬 문제, 취향을 구분한다. 국소 수정으로 해결되면 전체를 다시 쓰지 않는다. 사용자가 최종으로 확정한 원고는 명백한 오류가 없는 한 추가 교정을 강요하지 않는다.
